...
void Collecteur::modifier(Enregistrement* aDonnee)
{
	...
	aDonnee->getGeolocalisation(latitude,longitude);
	...
	laBDD->modifierEnregistrement(identifiantE,power,albedo,fluxLum, aDonnee->getBuboID(),lamp,streetlight,
				height,atof(latitude),atof(longitude),nbimages,town,aDonnee->getRue(),ULOR,laDate,notes);
	if (!listeEnregistrements.empty()) {
...
/***
* A corriger car erroné, faux : onne doit pas faire listeEnregistrements.back()->localiser(); mais
* récupérer l'enregistrement dont l'identifiant est passé en paramètre
**/
