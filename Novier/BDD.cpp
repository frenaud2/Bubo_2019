...
std::vector<Enregistrement*> BDD::selectData(string requetesql){
    ...
    if(!rc){
        ...
        //Vérifie qu'il y a des lignes disponibles
        ...
        while(rc == SQLITE_OK || rc == SQLITE_ROW){
            ...
            if(rc == SQLITE_ROW){
					...
					Enregistrement *enreg = new Enregistrement(atoi(record.at(0).c_str()), record.at(2), record.at(3), record.at(4), record.at(5), record.at(6), 
						record.at(7), atof(record.at(8).c_str()),  atof(record.at(9).c_str()), atoi(record.at(10).c_str()), record.at(11),record.at(12),
						 atoi(record.at(13).c_str()), record.at(14), record.at(15), atoi(record.at(1).c_str()));
...
vector<Enregistrement*> obtenirListeDesLampadairesProches(double latitude,double longitude)
{
...
Enregistrement* BDD::sauvegarderDonneesEnregistrement(Enregistrement *enregistrement,string identifiantCollecteur)
{
	...
	if( rc == SQLITE_OK ) 
	{
		...
		sqlite3_bind_text(stmt,14, laDate.c_str(), 6, 0);
...
/// Recherche du dernier enregistrement créé pour rafraîchissement automatique
    ...
    if(!rc)
    {
        ...
        if(rc == SQLITE_ROW)
        {
            ...
            // Mettre les valeurs dans l'objet enreg
           enregistrement = new Enregistrement(atoi(record.at(0).c_str()),record.at(2),record.at(3),record.at(4),record.at(5),record.at(6),record.at(7),atof(record.at(8).c_str()),atof(record.at(9).c_str()),atoi (record.at(10).c_str()),record.at(11),record.at(12),atoi(record.at(13).c_str()), record.at(14),record.at(15),atoi(record.at(1).c_str()));
        }
    }
    	sqlite3_finalize(stmt);
    	sqlite3_close(db);
...
void BDD::modifierEnregistrement(int id, int puissance, string albedo, string fluxLum, string idRaspbery, string typeAmpoule, string typeLampadaire, string hauteur, double latitude, double longitude, int nbImages, string agglo, string rue , int ulor, string date, string notes)
{
    ...
    const char *pzTest;
			...
			string mon_fichier = "/home/pi/testBDD.txt";
    ofstream fichier(mon_fichier.c_str(),  ios::out | ios::app  );
	std :: string sql = "UPDATE Enregistrements SET power=?, albedo=?, flux=? , buboid=?, lamp=?, streetlight=?, height=?, latitude=?, longitude=?, nbimages=?, town=?, street=?, ulor=?, date=?, notes=? where id=?";
	...
	if ( rc == SQLITE_OK) {
        ...
        sqlite3_bind_text(stmt,12,rue.c_str(),rue.length(),0);
        sqlite3_bind_int(stmt,13,ulor);
        sqlite3_bind_text(stmt,14,date.c_str(),6,0);
        sqlite3_bind_text(stmt,15,notes.c_str(),notes.length(),0);
